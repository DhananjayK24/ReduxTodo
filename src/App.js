...
import { AddTodoAction, RemoveTodoAction, SuccessTodoAction } from './actions/TodoActions';
      ...
      <h1 className="text-center text-white my-3 mb-4 text-5xl leading-none tracking-tight text-gray-900 md:text-4xl lg:text-6xl dark:text-white">Enter a <span className="text-blue-600 dark:text-blue-500">Todo</span></h1>
        ...
        <ul className='flex flex-col gap-2 items-start mx-auto max-w-md space-y-1 text-gray-500 list-inside dark:text-gray-400'>
          ...
          {todos.map((t)=>(
            ...
            >
              <svg onClick={()=>{dispatch(SuccessTodoAction(t))}} className={`w-5 h-5 me-2 ${t.success ? 'text-green-500 dark:text-green-400 flex-shrink-0' : 'text-gray-500 dark:text-gray-400 flex-shrink-0'}`} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20" style={{cursor: 'pointer'}}>
